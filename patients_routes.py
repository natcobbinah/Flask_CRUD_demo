    ...
    if user_id_to_edit is not None:
        ...
        if db.session.execute(stmt_patient_records).first() is not None:
            stmt = (
                ...
                .values(
                    (user_id_to_edit, form_data["FirstName"],  form_data["LastName"], form_data["DOB"],
                     form_data["Gender"], form_data["ContactNumber"], form_data["Address"])
                )
            )
            db.session.execute(stmt)
            db.session.commit()
        ...
        insert_current_record_to_patient_table = insert(Patients).values(
            (current_patient_table_index + 1, form_data["FirstName"],
             form_data["LastName"], form_data["DOB"], form_data["Gender"],
             form_data["ContactNumber"],  form_data["Address"]
             )
        )
